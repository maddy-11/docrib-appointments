...
trait AppointmentManager
{
    ...
    public function new()
    {
        ...
        $appointments = Appointment::newAppointment()->where(function ($query) {
            $query->whereNull('is_confirmed');
        })->with('staff', 'doctor', 'assistant');
    ...
    public function confirmedService()
    {
        $pageTitle    = 'Confirmed Appointments';
        $appointments = Appointment::where('is_confirmed', true)->where('is_complete', false)->with('staff', 'doctor', 'assistant');
        $appointments = $this->detectUserType($appointments);
        // dd($appointments);
        return view($this->userType . '.appointment.index', compact('pageTitle', 'appointments'));
    }
    ...
    public function serviceTrashed()
    {
        ...
        $appointments = Appointment::where(function($query) {
            $query->where('is_delete', Status::YES)
            ->orWhere('is_confirmed', false);
        });
        $appointments = $this->detectUserType($appointments);
        
        $appointments->load(['doctor', 'staff', 'assistant']);
        
        $deletedAppointments = $appointments->where('is_delete', Status::YES);
        $deletedAppointments->load(['deletedByStaff', 'deletedByDoctor', 'deletedByAssistant']);
